...
- **README demo GIFs are generated, not hand-recorded.** `docs/demo.gif` (TUI
  overview) and `docs/add.gif` (offline `soap add` flow) are regenerated by
  `uv run python scripts/demo.py`, which seeds a throwaway `HOME`/`SOAP_DIR`
  library (same fake-library idea as `shoot_tui.py`) and drives the real `soap`
  binary via VHS (`charmbracelet/vhs`) over `scripts/demo.tape` +
  `scripts/demo-add.tape`. Needs `vhs`+`ttyd`+`ffmpeg` on PATH. Fully offline
  and deterministic; leaks nothing real. Details: `docs/releasing.md`
